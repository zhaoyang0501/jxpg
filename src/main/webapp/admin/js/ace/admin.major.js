jQuery.adminMajor = {
		...
		initSearchDataTable : function() {
			if (this.majorDataTable == null) {
				this.majorDataTable = $('#dt_table_view').dataTable({
					...
					"bProcessing" : true,
					"bSort" : false,
					"sAjaxSource" : $.ace.getContextPath() + "/admin/major/list",
				...
				var oSettings = this.majorDataTable.fnSettings();
				oSettings._iDisplayStart = 0;
				this.majorDataTable.fnDraw(oSettings);
			}
		...
		deleteMajor :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/major/delete/"+id,
	        			dataType : "json",
	        			success : function(json) {
	        				if(json.state=='success'){
	        					noty({"text":""+ json.msg +"","layout":"top","type":"success","timeout":"2000"});
	        					$.adminMajor.initSearchDataTable();
	        				}else{
		...
		showEdit: function (id){
			...
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/major/get/"+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
		...
		saveMajor: function(id){
			$.ajax({
    			type : "post",
    			url : $.ace.getContextPath() + "/admin/major/save",
    			data:$("form").serialize(),
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
